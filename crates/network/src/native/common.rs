...
use crate::{client::ConnectionTransport, NetworkError, MAX_FRAME_SIZE};

impl ConnectionTransport for WebTransportSession<h3_quinn::Connection, Bytes> {
...
impl ConnectionTransport for quinn::Connection {
    fn request_bi(&self, id: u32, data: Bytes) -> BoxFuture<Result<Bytes, NetworkError>> {
        Box::pin(async move {
...
impl ConnectionTransport for crate::native::client_connection::ConnectionKind {
    fn request_bi(&self, id: u32, data: Bytes) -> BoxFuture<Result<Bytes, NetworkError>> {
        Box::pin(async move {
            let (mut send, mut recv) = self.open_bi().await?;
