...
impl WaterSimCompute {
    ...
    fn create_pipeline(gpu: &Gpu, entry_point: &str) -> wgpu::ComputePipeline {
        let shader = gpu.device.create_shader_module(wgpu::ShaderModuleDescriptor {
            ...
            source: wgpu::ShaderSource::Wgsl(Cow::Owned(wgsl_terrain_preprocess(include_file!("water_sim.wgsl")))),
        });
    ...
    pub fn run(&self, gpu: &Gpu, encoder: &mut wgpu::CommandEncoder, texture: &TextureView, size: UVec2, config: &WaterSimConfig) {
        ...
        for pipeline in [&self.rain, &self.flux, &self.update_water, &self.water_erosion, &self.sediment_movement, &self.thermal_erosion] {
