...
pub async fn object_pipelines_from_quixel_json(
    ...
    match get_path(quixel, vec!["semanticTags", "asset_type"])
        .unwrap()
        .as_str()
        .unwrap() as &str
    {
        "3D asset" => {
            let material = PbrMaterialDesc {
                base_color: Some(
                    pipe_image(&format!("{}_Albedo.jpg", quixel_id.resolution)).await?,
                ),
                ...
                normalmap: Some(
                    pipe_image(&format!("{}_Normal_LOD0.jpg", quixel_id.resolution)).await?,
                ),
                ...
                for i in 0..6 {
                    ...
                    lods.push(
                        ModelImportPipeline::new()
                            ...
                            .add_step(ModelImportTransform::Transform(ModelTransform::SetRoot {
                                name: root,
                            })),
                    );
                }
        ...
        "3D plant" => {
                    ...
                    path.as_array().unwrap().iter().find_map(|map| {
                        ...
                        if map.get("name").unwrap().as_str().unwrap() == name {
                            for uri in map.get("uris").unwrap().as_array().unwrap() {
                                for resolution in
                                    uri.get("resolutions").unwrap().as_array().unwrap()
                                {
                                        ...
                                        resolution.get("formats").unwrap().as_array().unwrap()
                                    {
                                        ...
                                        if format.get("mimeType").unwrap().as_str().unwrap()
                                            == "image/jpeg"
                                            && format
                                                .get("uri")
                                                .unwrap()
                                                ...
                                                .contains(&format!(
                                                    "_{}_",
                                                    quixel_id.resolution.to_uppercase()
                                                ))
                                        {
                                            return Some(
                                                format
                    ...
                    async move {
                        if let Some(ending) = ending {
                            Ok(Some(AssetUrl::from(
                                PipeImage::new(
                                    ctx.get_downloadable_url(&in_root_url.push(ending).unwrap())
                                        .unwrap()
                                        .clone(),
                                )
            ...
            let atlas = PbrMaterialDesc {
                base_color: pipe_image_opt(
                    get_map_v1("maps", "Albedo").or_else(|| get_map_v2("components", "Albedo")),
                )
                .await?,
                opacity: pipe_image_opt(
                    get_map_v1("maps", "Opacity").or_else(|| get_map_v2("components", "Opacity")),
                )
                .await?,
                normalmap: pipe_image_opt(
                    get_map_v1("maps", "Normal").or_else(|| get_map_v2("components", "Normal")),
                )
                .await?,
                alpha_cutoff: Some(0.5),
                metallic_roughness: pipe_mr_image(
                    get_map_v1("maps", "Roughness")
                    ...
                    == "lodDistance"
                {
                        ...
                        .unwrap()
                    {
                        ...
                        for (i, lod) in json_lods.iter().enumerate() {
                            ...
                            let lod_distance =
                                lod.get("lodDistance").unwrap().as_f64().unwrap() as f32;
                            lod_cutoffs.push(lod_distance * lod_factor);
                            let file = in_root_url
                                .push(format!(
                                    "Var{variation_nr}/Var{variation_nr}_LOD{lod_index}.fbx"
                                ))
                                .unwrap();
                            lods.push(
                                ModelImportPipeline::new()
            ...
            if object_defs.is_empty() {
                let mut variation = 0;
                loop {
                    if !ctx.files.has_input_file(
                        &in_root_url
                            .push(format!(
                                "Var{var}/Var{var}_LOD{lod}.fbx",
                                var = variation,
                                lod = 0
                            ))
                            .unwrap(),
                    ...
                    let lods = (0..6)
                        .map(|i| {
                            ModelImportPipeline::new().add_step(
                                ModelImportTransform::ImportModelFromUrl {
                                    url: ctx
                                        .get_downloadable_url(
                                            &in_root_url
                                                .push(format!(
                                                    "Var{variation}/Var{variation}_LOD{i}.fbx"
                                                ))
                                                .unwrap(),
                                        )
                                        .unwrap()
        ...
        _ => panic!(
            "Not implemented yet: {:?}",
            get_path(quixel, vec!["semanticTags", "asset_type"])
        ),
    }
    Ok(object_defs)
}
