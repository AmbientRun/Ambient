...
fn two_entities() {
    ...
    rt.block_on(async {
        ...
        let a = Entity::new().set(carrot(), vec4(7., 7., 3., 7.)).spawn(&mut test.world);
        test.update();

        let b = Entity::new().set(tomato(), vec4(1., 1., 3., 1.)).spawn(&mut test.world);
        test.update();
...
async fn gpu_ecs() {
    ...
    let _ignored = Entity::new().set(cpu_banana(), vec4(7., 7., 3., 7.)).spawn(&mut test.world);
