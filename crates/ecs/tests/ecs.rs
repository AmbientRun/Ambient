...
fn spawn_all_excl_query() {
    ...
    let _x = world.spawn(Entity::new().with(b(), 2.));
    assert_eq!(q.iter(&world, Some(&mut qs)).count(), 1);
    assert_eq!(q.iter(&world, Some(&mut qs)).count(), 0);
    world.spawn(Entity::new().with(a(), 1.));
    assert_eq!(q.iter(&world, Some(&mut qs)).count(), 0);
}
...
fn mirroring() {
    ...
    let id1b = EntityId(7);
    let id2 = EntityId(9);
    world.spawn_with_id(id1, Entity::new().with(a(), 3.));
    world.get(id1, a()).unwrap();
    assert_eq!(query((a(),)).iter(&world, None).count(), 1);
    world.spawn_with_id(id2, Entity::new().with(a(), 2.));
    world.get(id2, a()).unwrap();
    assert_eq!(query((a(),)).iter(&world, None).count(), 2);
    world.despawn(id1);
    assert_eq!(query((a(),)).iter(&world, None).count(), 1);
    world.spawn_with_id(id1b, Entity::new().with(a(), 3.));
    assert_eq!(query((a(),)).iter(&world, None).count(), 2);
    world.get(id2, a()).unwrap();
    world.despawn(id2);
    assert_eq!(query((a(),)).iter(&world, None).count(), 1);
