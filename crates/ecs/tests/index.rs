...
fn simple_index_exact() {
    ...
    assert_eq!(index.range(&start..&end).map(|x| x.id().unwrap()).collect_vec(), [x, dup]);

    let y = world.spawn(EntityData::new().set(a(), 3));
    index.insert_entity(&world, y);
    assert_eq!(index.range(&start..).map(|x| x.id().unwrap()).collect_vec(), [x, dup]);

    let z = world.spawn(EntityData::new().set(a(), 7));
    index.insert_entity(&world, z);
    assert_eq!(index.range(&start..).map(|x| x.id().unwrap()).collect_vec(), [x, dup, z]);
}
...
fn multiple_for_exact_value() {
    ...
    assert_eq!(index.range((start, end)).map(|x| x.id().unwrap()).collect_vec(), vec![y, z]);
}
