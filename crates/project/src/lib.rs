...
impl Version {
    ...
    pub fn new_from_str(id: &str) -> Result<Self, VersionError> {
        ...
        let (patch, suffix) = if let Some(patch) = segments.next() {
            ...
            (patch.parse()?, VersionSuffix::new_from_str(suffix.unwrap_or_default())?)
        } else {
...
impl<'de> Visitor<'de> for VersionVisitor {
    ...
    fn expecting(&self, formatter: &mut std::fmt::Formatter) -> std::fmt::Result {
        formatter.write_str("a semantic dot-separated version with up to three components and an optional prefix")
    }
