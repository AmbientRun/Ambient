...
fn main(@builtin(global_invocation_id) id: vec3<u32>, @builtin(num_workgroups) workgroups: vec3<u32>) {
    ...
    for (var i = 0u; i < arrayLength(&natural_elements.data); i = i + 1u) {
        ...
        element_probability[i] = clamp(sample_curve(element_conf.soil_depth, soil), 0., 1.) * clamp(sample_curve(element_conf.elevation, height_over_ocean), 0., 1.) * clamp(sample_curve(element_conf.water_depth, max(0., -height_over_ocean)), 0., 1.) * clamp(sample_curve(element_conf.steepness, steepness), 0., 1.) * clamp(sample_curve(element_conf.cluster_noise, cluster_noise), 0., 1.);
    }
    ...
    if element_probability[element] > threashold {
        ...
        output_entities.data[out_offset].position = vec3<f32>(position, elevation) + normal * element_conf.position_normal_offset + vec3<f32>(0., 0., element_conf.position_z_offset);
        ...
        output_entities.data[out_offset].rotation.z = atan2(normal.y, normal.x) + (2. * rot_z_jitter - 1.) * 3.14159 * element_conf.rotation_z_jitter + element_conf.rotation_z;
        output_entities.data[out_offset].rotation.x = -asin(normal.y) * (1. - element_conf.rotation_straightness) + (2. * rot_x_jitter - 1.) * 3.14159 * element_conf.rotation_xy_jitter + element_conf.rotation_x;
        output_entities.data[out_offset].rotation.y = asin(normal.x) * (1. - element_conf.rotation_straightness) + (2. * rot_y_jitter - 1.) * 3.14159 * element_conf.rotation_xy_jitter + element_conf.rotation_y;
