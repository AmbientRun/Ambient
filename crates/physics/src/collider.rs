        ...
        vec![
            ...
            query(collider_from_url().changed()).to_system(|q, world, qs, _| {
                for (id, url) in changed_or_missing(q, world, qs, collider()) {
                    ...
                    match TypedAssetUrl::parse(&url).context("Failed to parse url").and_then(|url: TypedAssetUrl<ColliderAssetType>| {
                        url.0.as_directory().join("colliders/main.json").context("Failed to join url")
                    }) {
                        Ok(url) => world.add_component(id, collider(), ColliderDef::Asset { collider: url.into() }).unwrap(),
