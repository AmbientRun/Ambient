...
async function run(samples, build, nParallel) {
    ...
    let errors = (await process(nParallel, samples.map(([path, seconds], index) => async () => {
        ...
        try {
            const command = build ? `build ${path}` : `run ${path} --headless --golden-image-test ${seconds} --quic-interface-port ${9000 + index} --http-interface-port ${10000 + index}`;
...
run(samples, false, samples.length);
