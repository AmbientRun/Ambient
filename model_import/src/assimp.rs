...
use elements::model::{pbr_renderer_primitives_from_url, Model, PbrRenderPrimitiveFromUrl};
use elements_core::{
...
pub async fn import_url(
    ...
    let extension = match ContentLoc::parse(url) {
        ...
        Ok(ContentLoc::Url(url)) => url.path().rsplit_once(".").map(|(a, b)| b.to_string()),
        Err(_) => None,
    }
    .unwrap_or_default();
    ...
    for (i, material) in materials.iter().enumerate() {
        ...
        for (key, texs) in &material.textures {
            if let Some(tex) = texs.get(0) {
                let image = if let Some(data) = &tex.data {
                    todo!()
                } else {
