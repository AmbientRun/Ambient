...
use elements::{animation::AnimationOutputs, model::Model, ElementEditor};
...
use elements_std::{
    asset_cache::{AssetCache, AsyncAssetKey, AsyncAssetKeyExt, SyncAssetKeyExt}, download_asset::{download, AssetResult, AssetsCacheDir, ContentLoc, ReqwestClientKey, UrlString}
};
...
impl ModelImportPipeline {
    ...
    pub async fn produce_crate(&self, assets: &AssetCache, base_url: impl Into<String>) -> anyhow::Result<ModelCrate> {
        let mut asset_crate = ModelCrate::new(base_url);
        for step in &self.steps {
            step.run(assets, &mut asset_crate).await.with_context(|| format!("Failed to run step: {:?}", step))?;
        }
        Ok(asset_crate)
    }
    pub async fn produce_local_model_url(&self, asset_cache: &AssetCache) -> anyhow::Result<String> {
        ...
        let model_crate = self
            ...
            .produce_crate(asset_cache, &format!("{}/", cache_path.to_str().unwrap()))
            .await?;
        model_crate.produce_local_model_url(asset_cache).await
    }
    ...
    pub async fn run(&self, assets: &AssetCache, model_crate: &mut ModelCrate) -> anyhow::Result<()> {
        match self {
            ...
            ModelImportTransform::MergeMeshLods { lods, lod_cutoffs } => {
                ...
                for lod in lods {
                    res_lods.push(lod.produce_crate(assets, ModelCrate::IN_MEMORY).await?);
                }
                model_crate
            ...
            ModelImportTransform::MergeUnityMeshLods { url, lod_cutoffs } => {
                let source = ModelImportPipeline::model(url).produce_crate(assets, ModelCrate::IN_MEMORY).await?;
...
pub async fn download_bytes(assets: &AssetCache, url: &str) -> anyhow::Result<Vec<u8>> {
    match ContentLoc::parse(url)? {
        ContentLoc::Url(url) => Ok(download(assets, url, |resp| async move { Ok(resp.bytes().await?) }).await?.into()),
        ContentLoc::RelativePath(path) => Ok(tokio::fs::read(path).await?),
    }
}
