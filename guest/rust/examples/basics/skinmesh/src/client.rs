...
pub fn main() {
    ambient_api::messages::Frame::subscribe(move |_, _| {
        ...
        if delta.keys.contains(&KeyCode::Key1) {
            messages::SetController::new(1u32).send(Target::ServerReliable);
        }

        if delta.keys.contains(&KeyCode::Key2) {
            messages::SetController::new(2u32).send(Target::ServerReliable);
        }

        if delta.keys.contains(&KeyCode::Key3) {
            messages::SetController::new(3u32).send(Target::ServerReliable);
        }
    });
}
