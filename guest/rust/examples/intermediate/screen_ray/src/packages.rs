...
mod raw {
    ...
    pub mod jkr622pbesmaaco76bjc6ll6v4t4lajh {
        pub fn entity() -> ambient_api::global::EntityId {
            use ambient_api::once_cell::sync::Lazy;
            static ENTITY: Lazy<ambient_api::global::EntityId> = Lazy::new(|| {
                ambient_api::package::get_entity_for_package_id("jkr622pbesmaaco76bjc6ll6v4t4lajh")
                    .expect("Failed to get package entity - was it despawned?")
            });
            *ENTITY
        }
        ...
        pub mod components {
            ...
            static PLAYER_CUBE_REF: Lazy<Component<EntityId>> = Lazy::new(|| {
                __internal_get_component("jkr622pbesmaaco76bjc6ll6v4t4lajh::player_cube_ref")
            });
            #[doc = "**Player Cube Ref**: Attached to the player; references the preview cube the player is controlling.\n\n*Attributes*: Networked, Debuggable"]
            pub fn player_cube_ref() -> Component<EntityId> {
                *PLAYER_CUBE_REF
            }
        }
        ...
        pub mod messages {
            ...
            impl Input {
                ...
                pub fn new(
                    ...
                    ray_dir: impl Into<Vec3>,
                    spawn: impl Into<bool>,
                ) -> Self {
                    Self {
                        ray_origin: ray_origin.into(),
                        ray_dir: ray_dir.into(),
                        spawn: spawn.into(),
                    }
                }
            }
            impl Message for Input {
                fn id() -> &'static str {
                    "jkr622pbesmaaco76bjc6ll6v4t4lajh::Input"
                }
                fn serialize_message(&self) -> Result<Vec<u8>, MessageSerdeError> {
                    let mut output = vec![];
                    self.ray_origin.serialize_message_part(&mut output)?;
                    self.ray_dir.serialize_message_part(&mut output)?;
                    self.spawn.serialize_message_part(&mut output)?;
                    Ok(output)
                }
                fn deserialize_message(mut input: &[u8]) -> Result<Self, MessageSerdeError> {
                    Ok(Self {
                        ray_origin: Vec3::deserialize_message_part(&mut input)?,
                        ray_dir: Vec3::deserialize_message_part(&mut input)?,
                        spawn: bool::deserialize_message_part(&mut input)?,
                    })
                }
            }
...
pub use raw::jkr622pbesmaaco76bjc6ll6v4t4lajh as this;
